...
local uselessEnglishWords = {
    ...
    xiloveskrylorspaghettix = true,
    trallalero = true,
    ineskiwistopspamming = true,
}


local usefulEnglishWords = {
    larp = true,
    larping = true,
    larper = true,
    larps = true,
...
local function LoadList(fname)
    ...
    if isfile(fname) then
        ...
        for w in content:gmatch("[^\r\n]+") do
            ...
            if #clean > 0 and not SeenWords[clean] and not uselessEnglishWords[clean] then
...
for word in pairs(usefulEnglishWords) do
